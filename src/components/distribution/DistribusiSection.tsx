...
export const DistribusiSection: React.FC<DistribusiSectionProps> = ({ date, location }) => {
  ...
  const loadAttendanceData = async () => {
    if (!location.trim()) {
      toast.error('Isi Sesi / Lokasi Kegiatan terlebih dahulu');
      return;
    }

    setIsLoadingAttendance(true);
    try {
      const data = await getAttendanceWithMembers(date, location);

      if (!data || data.length === 0) {
        toast.error('Tidak ada anggota hadir/tampil untuk sesi ini');
      ...
      toast.success(`${data.length} anggota dimuat`);
    } catch (err) {
  ...
  const handleExportDetailPDF = async () => {
    ...
    const { distribution, recipients: detailRecipients } = selectedDetail;
    ...
    const columns = ['No', 'Nama Anggota', 'Status Kehadiran', 'Nominal Diterima', 'Status Pembayaran'];
    const detailData = detailRecipients.map((r, i) => [
      String(i + 1).padStart(2, '0'),
      memberName(r),
    ...
    const totalDistributed = detailRecipients.reduce((s, r) => s + r.amount, 0);
    const lunasCountDetail = detailRecipients.filter(r => r.amount > 0).length;
    const belumCount = detailRecipients.filter(r => r.amount === 0).length;
    ...
    const foot = [
      [
        ...
        `${lunasCountDetail} Lunas | ${belumCount} Belum`
      ]
    ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 pt-4">
        ...
        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Distribusi Dana</span>
        <div className="flex-1 h-px bg-gray-200" />
      </div>
      ...
      <div className="p-4 md:p-5 bg-white border border-gray-200 rounded-xl shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5">Sesi</label>
            <div className="flex items-center gap-2">
              <div className="flex-1 flex items-center gap-2.5 px-3 py-2 bg-gray-50 border border-gray-200 rounded-md">
                <Calendar size={14} className="text-gray-400 shrink-0" />
                <p className="text-sm font-semibold text-gray-700">{date} {location ? `• ${location}` : ''}</p>
              </div>
              <Button
                onClick={loadAttendanceData}
                isLoading={isLoadingAttendance}
                variant="outline"
                className="h-[34px] text-[10px] font-bold px-3 shrink-0"
              >
      ...
      {!isDataLoaded && (
        <div className="bg-white border border-dashed border-gray-200 rounded-xl p-8 text-center">
          ...
          <p className="text-sm font-semibold text-gray-600">Belum ada data dimuat</p>
          <p className="text-xs text-gray-400 mt-1">Klik "Muat" untuk mengambil data absensi sesi ini</p>
        </div>
      )}
